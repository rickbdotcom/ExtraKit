...
// Keypath support - rickb
public extension Decoder {

    func defaultDecodeIfPresent(_ keyPath: String, _ default: String = "") -> String {
        return defaultDecodeIfPresent(keyPath, as: String.self, default: `default`)
    }

    func defaultDecodeIfPresent(_ keyPath: String, _ default: Int = 0) -> Int {
        return defaultDecodeIfPresent(keyPath, as: Int.self, default: `default`)
    }

    func defaultDecodeIfPresent(_ keyPath: String, _ default: Bool = false) -> Bool {
        return defaultDecodeIfPresent(keyPath, as: Bool.self, default: `default`)
    }

    func defaultDecodeIfPresent(_ keyPath: String, _ default: Double = 0.0) -> Double {
        return defaultDecodeIfPresent(keyPath, as: Double.self, default: `default`)
    }

    func defaultDecodeIfPresent(_ keyPath: String, _ default: Int64 = 0) -> Int64 {
        return defaultDecodeIfPresent(keyPath, as: Int64.self, default: `default`)
    }

    func defaultDecodeIfPresent<T: Decodable>(_ keyPath: String, _ default: [T] = []) -> [T] {
        return defaultDecodeIfPresent(keyPath, as: [T].self, default: `default`)
    }

    func defaultDecodeIfPresent<T: Decodable>(_ keyPath: String, as type: T.Type = T.self, default: T? = nil) -> T? {
        return (try? decodeIfPresent(keyPath: keyPath, as: type) ?? `default`) ?? `default`
    }

    func defaultDecodeIfPresent<T: Decodable>(_ keyPath: String, as type: T.Type = T.self, default: T) -> T {
        return (try? decodeIfPresent(keyPath: keyPath, as: type) ?? `default`) ?? `default`
    }

	func decodeIfPresent<T: Decodable>(keyPath: String, as type: T.Type = T.self) /*throws*/ -> T? {
do {
		var container: AnyDecodingContainer?
		let keys = keyPath.components(separatedBy: ".")
		container = .keyed(try self.container(keyedBy: AnyCodingKey.self))
		...
		if let container = container, let key = keys.last {
			...
			case let .unkeyed(unkeyedContainer):
				...
				if let index = Int(key) {
					for _ in 0..<index-1 {
						_ = try container.decodeIfPresent(type)
					}
...
enum AnyDecodingContainer {
	...
	func nestedContainer(forKey key: String) throws -> AnyDecodingContainer? {
		...
		case let .keyed(container):
			if Int(key) != nil {
				return .unkeyed(try container.nestedUnkeyedContainer(forKey: AnyCodingKey(key)))
			} else {
				return .keyed(try container.nestedContainer(keyedBy: AnyCodingKey.self, forKey: AnyCodingKey(key)))
			}
		case let .unkeyed(unkeyedContainer):
			var container = unkeyedContainer
			if let index = Int(key) {
				...
				return .keyed(try container.nestedContainer(keyedBy: AnyCodingKey.self))
			}
		}
		return nil
	}
