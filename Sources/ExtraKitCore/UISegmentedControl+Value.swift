...
extension UISegmentedControl {
	...
	func selectedValue<T: Equatable>(as type: T.Type = T.self) -> T? {
	...
	func setValues<T: Equatable>(_ values: [T], title: (T) -> String?) {
		valuesStorage = values
		removeAllSegments()
		values.reversed().forEach { value in
			insertSegment(withTitle: title(value), at: 0, animated: false)
		}
	}
